...
#define OUT_PORT1 0xe000
#define OUT_PORT2 0xe004
...
{
	CARD32 val;
	/*if (port != OUT_PORT1 && port != OUT_PORT2)
		fprintf(stderr,"INREG(0x%04X)\n =", port);*/
	val = inl_local(port);
	/*if (port != OUT_PORT1 && port != OUT_PORT2)
		fprintf(stderr,"=%08X\n", val);
	if (port == OUT_PORT2 && !(curr_reg & 0x80000000))
		fprintf(stderr,"INREG(0x%08X); \n = %08X\n", curr_reg, val);*/
	return val;
}
...
{
	/*if (port != OUT_PORT1 && port != OUT_PORT2)
		fprintf(stderr,"outl %04X, %08X\n", port, val);*/

	outl_local(port, val);
	if (port == OUT_PORT1)
		curr_reg = val;
	/*if (port == OUT_PORT2 && !(curr_reg & 0x80000000))
		fprintf(stderr,"OUTREG(0x%08X, 0x%08X);\n", curr_reg, val);*/
}
...
int LRMI_init() {
	...
	M.mem_base = 0;
        /*        M.x86.debug |= DEBUG_MEM_TRACE_F | DEBUG_TRACECALL_F;*/
	return 1;
}
